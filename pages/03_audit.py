...
with st.form("audit_form"):
    ...
    raise_actions = st.toggle("Create action items for any FAIL results", value=True)
...
if submitted:
    ...
    if raise_actions:
        for row in data:
            if row["result"] == "Fail":
                add_action(source="Audit", source_id=0, site=site, title=f"Fix: {row['item']}")
                actions_raised += 1

    aid = save_audit(site=site, auditor=auditor or None, date_iso=date_iso, checklist=json.dumps(data), findings=findings or None, actions_raised=actions_raised)
